...
def build_run(source_master: Path, runs_dir: Path, run_id: str, assignment_count: int = 900, seed: int = 42, mvp_store_count: int | None = None) -> Path:
    ...
    manifest = {"run_id": run_id, "master_sha256": master.sha256, "seed": seed, "scenario_count": len(scenarios), "assignment_count": len(assignments), "mvp_store_count": len(selected), "annotator_counts": assignments.annotator_id.value_counts().sort_index().to_dict(), "distribution_policy": distribution_policy, "generated_counts": {column: scenarios[column].fillna("NONE").value_counts().sort_index().to_dict() for column in ["day_of_week", "visit_time", "companion_type", "available_time_code", "budget_code", "purpose_code", "atmosphere_code", "hill_preference", "parking_preference"]}}
