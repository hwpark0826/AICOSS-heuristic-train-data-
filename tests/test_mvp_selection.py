...
from src.mvp_selection import EXCLUDED_MVP_CATEGORY_SUBS, EXCLUDED_MVP_STORE_IDS, load_mvp_store_roster, select_mvp_stores
...
def test_mvp_selection_is_diverse_and_has_requested_size() -> None:
    ...
    assert tuple(selected.store_id) == roster_ids
...
def test_fixed_mvp_roster_fails_when_a_reviewed_store_is_inactive() -> None:
    master = load_master(find_master_file(Path("data/master")))
    roster_ids = tuple(load_mvp_store_roster().store_id)
    tables = dict(master.tables)
    stores = tables["STORE"].copy()
    stores.loc[stores.store_id.eq(roster_ids[0]), "active_yn"] = "N"
    tables["STORE"] = stores

    with pytest.raises(ValueError, match=roster_ids[0]):
